        ...
        *,
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as pool:
            ...
            while sorter.is_active() or future_to_info:
                ...
                schedule_ready_nodes()
                # Also schedule any deferred nodes that became eligible due to resolved cases
                if deferred_ready:
                    for tid in list(deferred_ready):
                        cid = target_to_case.get(tid)
                        if cid is None or cid not in resolved_cases:
                            continue
                        selected = allowed_target_for_case.get(cid)
                        if selected == "__CONST__":
                            # Case decided to return a constant; this branch should be skipped and finalized
                            disabled_nodes.add(tid)
                            logger.debug(
                                "[DEFERRED] finalize skipped branch %s due to constant case %s",
                                tid,
                                cid,
                            )
                            if event_sink and run_id:
                            ...
                            deferred_ready.discard(tid)
                            continue
                        if selected is None or selected != tid:
                            continue
                        nd = graph.nodes[tid]
                        deferred_sig = inspect.signature(nd.func)
                        deferred_kwargs: Dict[str, Any] = {}
                        for an, dep in nd.dependencies.items():
                            deferred_kwargs[an] = _resolve_dep_value(dep)
                        for pn, prm in deferred_sig.parameters.items():
                            if pn in deferred_kwargs:
                                continue
                            dv = prm.default
                            if dv is not inspect._empty:
                                from ..core import CONTEXT as CONTEXT_SENTINEL
                                ...
                                if dv is CONTEXT_SENTINEL:
                                    deferred_kwargs[pn] = context
                                    continue
                            if pn in results:
                                deferred_kwargs[pn] = results[pn]
                        for ck, cv in nd.meta.get("const_bindings", {}).items():
                            deferred_kwargs.setdefault(ck, cv)
                        if event_sink and run_id:
                            try:
                                from ..services.telemetry import NodeEvent

                                event_sink.on_node_event(
                                    NodeEvent(run_id, tid, "scheduled", time.time())
                                )
                            except Exception:
                                pass
                        if event_sink and run_id:
                            try:
                                ...
                                event_sink.on_node_event(
                                    NodeEvent(run_id, tid, "started", time.time())
                                )
                            except Exception:
                                pass
                        logger.debug(
                            "[DEFERRED] scheduling %s now that case %s resolved",
                            tid,
                            cid,
                        )
                        fut2 = pool.submit(
